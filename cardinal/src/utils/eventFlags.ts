...
const ACTION_FLAGS: ReadonlyArray<FlagEntry> = [
  { bit: FLAG_BITS.ITEM_CREATED, label: 'Created', raw: 'ItemCreated' },
  { bit: FLAG_BITS.ITEM_REMOVED, label: 'Removed', raw: 'ItemRemoved' },
  { bit: FLAG_BITS.ITEM_RENAMED, label: 'Renamed', raw: 'ItemRenamed' },
  { bit: FLAG_BITS.ITEM_MODIFIED, label: 'Modified', raw: 'ItemModified' },
  { bit: FLAG_BITS.ITEM_CLONED, label: 'Cloned', raw: 'ItemCloned' },
  { bit: FLAG_BITS.ITEM_INODE_META_MOD, label: 'InodeMetaMod', raw: 'ItemInodeMetaMod' },
  { bit: FLAG_BITS.ITEM_FINDER_INFO_MOD, label: 'FinderInfoMod', raw: 'ItemFinderInfoMod' },
  { bit: FLAG_BITS.ITEM_CHANGE_OWNER, label: 'ChangeOwner', raw: 'ItemChangeOwner' },
  { bit: FLAG_BITS.ITEM_XATTR_MOD, label: 'XattrMod', raw: 'ItemXattrMod' },
];
...
const SYSTEM_FLAGS: ReadonlyArray<FlagEntry> = [
  { bit: FLAG_BITS.ROOT_CHANGED, label: 'RootChanged', raw: 'RootChanged' },
  { bit: FLAG_BITS.MOUNT, label: 'Mount', raw: 'Mount' },
  { bit: FLAG_BITS.UNMOUNT, label: 'Unmount', raw: 'Unmount' },
  { bit: FLAG_BITS.MUST_SCAN_SUBDIRS, label: 'MustScanSubDirs', raw: 'MustScanSubDirs' },
  { bit: FLAG_BITS.USER_DROPPED, label: 'UserDropped', raw: 'UserDropped' },
  { bit: FLAG_BITS.KERNEL_DROPPED, label: 'KernelDropped', raw: 'KernelDropped' },
  { bit: FLAG_BITS.EVENT_IDS_WRAPPED, label: 'EventIdsWrapped', raw: 'EventIdsWrapped' },
  { bit: FLAG_BITS.HISTORY_DONE, label: 'HistoryDone', raw: 'HistoryDone' },
] as const;

const DISPLAY_FLAGS: ReadonlyArray<FlagEntry> = [
  ...ACTION_FLAGS,
  ...SYSTEM_FLAGS,
  { bit: FLAG_BITS.OWN_EVENT, label: 'OwnEvent', raw: 'OwnEvent' },
];
...
export function describeEventFlags(flagBits?: number | null): EventFlagDescription {
  if (typeof flagBits !== 'number' || !Number.isFinite(flagBits)) {
    return { labels: ['—'], tooltip: 'No event flags' };
  }
  ...
  const labels = orderedEntries
    .filter((entry) => hasFlag(flagBits, entry.bit))
  ...
  const matchedFlagNames = DISPLAY_FLAGS.filter((entry) => hasFlag(flagBits, entry.bit)).map(
    (entry) => entry.raw,
  );

  const unknownBits = flagBits & ~ALL_FLAG_MASK;
  if (unknownBits) {
    matchedFlagNames.push(`0x${unknownBits.toString(16)}`);
  }

  const tooltip =
    matchedFlagNames.length > 0
      ? `${matchedFlagNames.join(', ')} (${flagBits})`
