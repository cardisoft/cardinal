...
- 每个查询都会被解析成一棵树，由以下元素组成：
  ...
  - **布尔运算符**（`AND`, `OR`, `NOT` / `!`）。
- 匹配是 **按路径片段** 进行的：
  - 普通词、短语和通配符词元匹配文件或文件夹自己的名称。
  - 带 `/` 的词元匹配一串连续的路径片段，并返回匹配最后一个片段的项目。
...
快速示例：
```text
report draft                  # 自身名称同时包含 “report” 和 “draft” 的文件或文件夹
...
- 和普通词元一样，通配符词元匹配路径片段。`src/**/Cargo.toml` 这样的斜杠链会返回匹配到的 `Cargo.toml` 项，而 `src/**` 返回匹配到的 `src` 文件夹下的子孙项。
- 若需要字面量 `*` 或 `?`，请对词元加引号：`"*.rs"`。Globstar 必须是独立的斜杠片段（`foo/**/bar`, `/Users/**`, `**/notes`）。

### 2.3 使用 `/` 的路径式分段

Cardinal 能理解词元中的“斜杠分段”，并将每个分段归类为路径片段的前缀/后缀/精确/子串匹配。示例：
...
#  自身名称包含 “Application Support” 的项目
"Application Support"
