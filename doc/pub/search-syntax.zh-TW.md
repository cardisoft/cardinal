...
- 每個查詢都會被解析成一棵樹，由以下元素組成：
  ...
  - **布林運算子**（`AND`, `OR`, `NOT` / `!`）。
- 比對是 **按路徑片段** 進行的：
  - 一般詞、片語和萬用字元詞元會比對檔案或資料夾自己的名稱。
  - 帶 `/` 的詞元會比對一串連續的路徑片段，並回傳符合最後一個片段的項目。
...
快速範例：
```text
report draft                  # 自身名稱同時包含 “report” 與 “draft” 的檔案或資料夾
...
- 和一般詞元一樣，萬用字元詞元會比對路徑片段。`src/**/Cargo.toml` 這樣的斜線鏈會回傳匹配到的 `Cargo.toml` 項目，而 `src/**` 會回傳匹配到的 `src` 資料夾下的子孫項。
- 若需要字面量 `*` 或 `?`，請將詞元加上引號：`"*.rs"`。Globstar 必須是獨立的斜線片段（`foo/**/bar`, `/Users/**`, `**/notes`）。

### 2.3 使用 `/` 的路徑式分段

Cardinal 能理解詞元中的「斜線分段」，並將每個分段歸類為路徑片段的前綴/後綴/精確/子字串比對。範例：
...
`regex:` 會將詞元剩餘部分視為套用到路徑片段（檔名或資料夾名）的正則表達式。
...
#  自身名稱包含 “Application Support” 的項目
"Application Support"
