...
- 모든 쿼리는 다음으로 이루어진 트리로 파싱됩니다:
  ...
  - **불리언 연산자** (`AND`, `OR`, `NOT` / `!`).
- 매칭은 **경로 구성 요소** 단위로 수행됩니다. 일반 단어, 구문, 와일드카드는 파일이나 폴더 자신의 이름에 매칭되고, `/`로 구분된 토큰은 연속된 경로 구성 요소 체인에 매칭되어 마지막 세그먼트와 일치한 항목을 반환합니다.
...
- 따옴표 없는 토큰은 하나의 경로 구성 요소에 대한 **부분 문자열 매칭**입니다:
